...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # If no pad_token, set to eos_token
    ...
    def _generate_and_score_completions(
        ...
        # VLLM inference does not require truncation
        ...
        self._metrics[mode]["advantages_std"].append(advantages.std().item())
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.beta != 0.0:
            ...
            # k2
            per_token_kl = 0.5 * (ref_per_token_logps - per_token_logps) ** 2
        ...
        # Add more metrics
        mean_per_token_loss1 = ((per_token_loss1 * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
        mean_per_token_kl = ((self.beta * per_token_kl * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
        self._metrics[mode]["per_token_loss1"].append(mean_per_token_loss1.item())
        self._metrics[mode]["per_token_kl"].append(mean_per_token_kl.item())

        # Add more metrics
        mean_ref_per_token_logps = ((ref_per_token_logps * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
        self._metrics[mode]["ref_per_token_logps"].append(mean_ref_per_token_logps.item())
        mean_per_token_logps = ((per_token_logps * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
        self._metrics[mode]["per_token_logps"].append(mean_per_token_logps.item())
